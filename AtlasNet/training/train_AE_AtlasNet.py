...
parser.add_argument('--env', type=str, default =f"AE_AtlasNet_add_cup_{time}"   ,  help='visdom environment')
...
parser.add_argument('--log_folder', type=str, default =f'AE_AtlasNet_add_cup_{time}', help='saved_log_folder')
...
for epoch in range(opt.nepoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader_test, 0):#! dataloader_test 1/2
            points, cat, _ , _ = data
            points = points.transpose(2,1).contiguous()
            points = points.cuda()
            #SUPER_RESOLUTION
    ...
    print(log_table)
    for item in dataset_test.cat:
        print(item, dataset_test.perCatValueMeter[item].avg, dataset_test.perCatValueMeter[item].count, dataset_test.perCatValueMeter[item].val, dataset_test.perCatValueMeter[item].sum)
    ...
    #save last network
    print('saving net...')
    #torch.save(network.state_dict(), '%s/network.pth' % (dir_name), _use_new_zipfile_serialization=False)
    if os.path.isdir(dir_name):
        # 如果是文件夹，则拼接路径并保存模型
        model_path = os.path.join(dir_name, 'atlasnet_add_cup.pth')
