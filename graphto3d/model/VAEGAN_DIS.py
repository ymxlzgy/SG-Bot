...
class Sg2ScVAEModel(nn.Module):
    ...
    def decoder(self, z, objs, triples, attributes, manipulate=False):
        ...
        if self.use_angles:
            ...
            return d3_pred
    '''
    def decoder_with_changes_placebo(self, z, dec_objs, dec_triples, attributes, missing_nodes, manipulated_nodes):
        # append zero nodes
        nodes_added = []
    ...
    def get_keetps(self, dec_objs, missing_nodes, manipulated_nodes):
        ...
        return keep
    '''
    def decoder_with_changes(self, z, dec_objs, dec_triples, attributes, missing_nodes, manipulated_nodes, distribution=None):
        # append zero nodes
        if distribution is not None:
            (mu, cov) = distribution
        nodes_added = []
    ...
    def forward(self, enc_objs, enc_triples, enc, attributes, enc_objs_to_scene, dec_objs,
                dec_triples, dec, dec_attributes, dec_objs_to_scene, missing_nodes, manipulated_nodes,
                enc_angles=None, dec_angles=None):

        mu, logvar = self.encoder(enc_objs, enc_triples, enc, attributes, enc_angles)
        if self.use_AE:
            z = mu
        else:
            # reparameterization
