...
class Normalization(Operation):
    ...
    def center_bounding_box(self):
        ...
        :param points: torch Tensor Batch, N_pts, D_dim
        :return: diameter
        这是一个名为center_bounding_box的方法，它的作用是将点云数据（self.points）平移到一个新的位置，使得点云数据的中心位于坐标原点。这个方法有两个主要步骤：
        计算点云数据在每个维度上的最小值（min_vals）和最大值（max_vals）。
        计算点云数据在每个维度上的中心点坐标，即(min_vals + max_vals) / 2。
        将点云数据平移到新的位置，使得中心点坐标为坐标原点。这是通过调用self.translate(-(min_vals + max_vals) / 2)实现的。
        """
    ...
    def normalize_bounding_box(self, isotropic=True):#! 
        ...
        :param points: torch Tensor Batch, N_pts, D_dim
        这是一个名为normalize_bounding_box的方法，它的作用是将点云数据（self.points）归一化到一个边长为1的边界框内。方法包括以下步骤：
        首先，调用self.center_bounding_box()将点云数据的中心平移到坐标原点，并获取点云数据的直径（diameter）。
        如果isotropic参数为True（默认值），则要求归一化过程是各向同性的（即保持点云数据的长宽高比例不变）。
        在这种情况下，使用点云数据最大直径（torch.max(diameter, 2, keepdim=True)）作为缩放因子。如果isotropic为False，则分别对每个维度进行归一化。
        使用计算得到的缩放因子（diameter）对点云数据进行缩放，这是通过调用self.scale(1.0 / diameter)实现的。这样，点云数据会被归一化到一个边长为1的边界框内。
        :return:
        """
        ...
        points = self.points.numpy()#! 修改
        return np.squeeze(points)
    
    @staticmethod
    def normalize_bounding_box_functional(points):#! pointcloud_processor.Normalization.normalize_bounding_box_functional
        points = torch.tensor(points)#!修改
        operator = Normalization(points, inplace=False)
