...
g++ -std=c++14 -shared -o tf_sampling_so.so tf_sampling.cpp \
...
nvcc -std=c++14 -c -o tf_grouping_g.cu.o tf_grouping_g.cu \
 ${CUDA_INCLUDE} ${TF_CFLAGS} -D GOOGLE_CUDA=1 -x cu -Xcompiler -fPIC

g++ -std=c++14 -shared -o tf_grouping_so.so tf_grouping.cpp \
...
cd ../3d_interpolation
g++ -std=c++14 tf_interpolate.cpp -o tf_interpolate_so.so -shared -fPIC -shared -fPIC ${TF_CFLAGS} ${TF_LFLAGS} -O2
