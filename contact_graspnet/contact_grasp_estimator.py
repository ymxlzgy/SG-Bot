...
try:
    ...
    from contact_graspnet.data import farthest_points, distance_by_translation_point, preprocess_pc_for_inference, regularize_pc_point_count, reject_median_outliers
    tf.disable_eager_execution()
    TF2 = True
except:
    print("No tensorflow. check if it is robot mode.")
...
def extract_point_clouds(depth, K, segmap=None, rgb=None, z_range=[0.2, 1.8], segmap_id=0,
    ...
    if segmap is not None:
        ...
        for i in obj_instances:
            ...
            pc_segments[i] = pc_segment[(pc_segment[:, 2] < z_range[1]) & (pc_segment[:, 2] > z_range[0])]
...
class GraspEstimator:
    ...
    def __init__(self, cfg):
        ...
        self.get_bn_decay = get_bn_decay
    ...
    def build_network(self):
        ...
        step = tf.Variable(0)
        bn_decay = self.get_bn_decay(step, global_config['OPTIMIZER'])
