...
def inference(global_config, 
    ...
    # ros interface
    graspnet_ros = GraspNetRosInterface(sub_topic_name_pc="/newcrfs/depth/points")
    while not rospy.is_shutdown():
        ...
        if graspnet_ros.get_trigger_grasp_get():
            ...
            # hecalires_yaml_path = "/media/hyunjun/Research/grasping_ws/mono_grasping_ws/handeye_cali_results.yaml"
            # with open(hecalires_yaml_path, 'r') as f:
            #     cali_res = yaml.load(f, Loader=yaml.FullLoader)
            # c2w = np.array(cali_res["cali_mtx"]).reshape(4,4).transpose()
            
            parent_frame = "panda_link0"
            child_frame = "polarization_frame" # TODO
            c2w = graspnet_ros.get_posemat_from_tf_tree(parent_frame=parent_frame, child_frame=child_frame)
            
            
            # # easy handeye kinect calib result # TODO
            ...
            pc_full, pc_colors, _ = graspnet_ros.get_pc_info(c2w=c2w, 
                                                            view_filter=1.2,     # TODO to be configured
                                                            height_filter=-0.2,   # TODO to be configured
